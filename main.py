...
def call_real_api(number, api_type):
    """Call real APIs and get data, replace developer names with @Danger_devil1917"""
    ...
    for api in REAL_APIS[api_type]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 🔁 Recursively replace developer names in entire data
                def replace_dev_names(obj):
                    if isinstance(obj, dict):
                        new_dict = {}
                        for k, v in obj.items():
                            # Replace string values containing the old handles
                            if isinstance(v, str):
                                v = v.replace('@Owner_004', '@Danger_devil1917')
                                v = v.replace('@abhigyan_codes', '@Danger_devil1917')
                                # Also replace other possible variations if any
                                v = v.replace('Owner_004', 'Danger_devil1917')
                                new_dict[k] = v
                            elif isinstance(v, list):
                                new_dict[k] = [replace_dev_names(item) for item in v]
                            elif isinstance(v, dict):
                                new_dict[k] = replace_dev_names(v)
                            else:
                                new_dict[k] = v
                        return new_dict
                    elif isinstance(obj, list):
                        return [replace_dev_names(item) for item in obj]
                    elif isinstance(obj, str):
                        return obj.replace('@Owner_004', '@Danger_devil1917').replace('@abhigyan_codes', '@Danger_devil1917')
                    else:
                        return obj
                
                data = replace_dev_names(data)
